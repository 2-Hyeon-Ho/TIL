백준 2606번 바이러스  
신종 바이러스인 웜 바이러스는 네트워크를 통해 전파된다. 한 컴퓨터가 웜 바이러스에 걸리면 그 컴퓨터와 네트워크 상에서 연결되어   
...
키워드 : DFS와 BFS 둘다 해결이 가능하다. 인접행렬을 이용해 풀이가 가능하다.  
DFS는 모든 경우의 수를 탐색하고자 하는 미로 문제 같은 경우에 적합하고 BFS는 두 지점 사이의 최단 경로를 찾는 문제에 적합하다